...
#'
#' @description This function is intended to compare a new genomics dataset to
...
#' @param cquery.p Query p-values. A vector, a matrix with two columns where the
...
#' @param reference.p Reference p-values. A vector, a matrix with two columns where the first column
...
#' @param query.gL If query.p is a vector of p-values, this parameters is expected to contain a vector
#'		of corresponding gene IDs of same length.  Otherwise it is ignored.
#' @param reference.gL If reference.p is a vector of p-values, this parameters is expected to contain a vector
#'		of corresponding gene IDs of same length.  Otherwise it is ignored.
#' @param na.rm If TRUE, any NA's in query.p and/or reference.p are removed before computing the GRS score.
#' @param estimateNullDistr If TRUE, an empirical distribution of the gene-specific GRS score is estimated.
#' @param nullDistrQuantiles If 'estimateNullDistr' is TRUE, specifies the desired quantiles of the null-distribution.
#' @param nullDistrN If 'estimateNullDistr' is TRUE, specifies the number of iterations for the estimation function.
#' @param tolerateWarnings If FALSE, function returns (with the warning message as character string).
#'		This parameter is intended when using GRS() by the Genomics Portals.
#' @param pAdjust.method.query If NULL, the probability of differential expression is set to posterior probability of
#'		diff expression based on query.p, else 1-adjusted pvalue is used from the p.adjust function.
#' @param pAdjust.method.reference If NULL, the probability of differential expression is set to posterior probability
#'		of diff expression based on reference.p, else 1-adjusted pvalue is used from the p.adjust function.
#' @param lambda To be passed to qvalue() function which estimates the proportion of non-differentially expressed genes.
#' @param rescale If TRUE (default) quantile normalization is performed between scores and probabilities of
...
#'
#' @details This function is intended to compare a new genomics dataset to existing primary genomics data by using the Generalized Random Set approach to compute the concordance of gene lists based on corresponding p-values.
#'
#' @author Johannes Freudenberg, Mehdi Fazel-Najafabadi
#'
#' @references ...
#' @seealso ‘convertGeneTable’, ‘runCLEAN’, ‘LRpath’
#' @keywords annotation
...
tregGRS <- function(query.p, reference.p, query.gL=NULL, reference.gL=NULL, na.rm=TRUE,
	...
	estimateH0 <- function(p, lambda) {
		pi0 <- 1
		tryCatch(pi0 <- qvalue::qvalue(p, lambda=lambda)$pi0, error = function(e) {pi0 <- 1})
		pi0 <- min(pi0, 0.999)
		pi0/(1-pi0)
	}
	...
	if(estimateNullDistr) {
		...
		geneTable[which(geneTable[,3]==0),3] <- 1/(dim(temp)[1]+1)
    		geneTable[,4] <- qvalue::qvalue(geneTable[,3])$qvalues
